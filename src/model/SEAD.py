...
class SEAE(nn.Module):
    def __init__(self, args, queue_size=65536, momentum=0.999, temperature=0.07):
        ...
        Additionally, too inspiration from: https://github.com/facebookresearch/moco.
        ...
        self.encoder_q = getattr(models, args.model)(
            args, args.mode, num_classes=128)  # Query Encoder

        self.encoder_k = getattr(models, args.model)(
            args, args.mode, num_classes=128)  # Key Encoder
        ...
        # Create the queue to store negative samples  #用纯偏差网络时将128改成2，使之输出的编码特征和动量编码器的维度符合，动量编码器的维度在这里控制。
        self.register_buffer("queue", torch.randn(self.queue_size, 2))
    ...
    def forward(self, x_q, x_k):
        ...
        feat_q, score, sub_result, mse = self.encoder_q(x_q)
        ...
        with torch.no_grad():
            # Update the key encoder #把encoder_q的参数经过计算更新到encoder_k上
            ...
            feat_k, score, sub_result, mse1 = self.encoder_k(x_k)
